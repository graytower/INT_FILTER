...
def main():
    ...
    if source == '00:00:00:01:01:01':
	    ...
	    # dst4
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:01:02:02', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x01' / '\x00\x02' / '\x00\x01' / '\x00\x04' / '\x80\x04' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst2
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:01:01:02', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x02' / '\x00\x03' / '\x80\x04' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst3
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:02:02:02', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x02' / '\x00\x01' / '\x00\x02' / '\x00\x04' / '\x80\x04' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst5
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:02:01:01', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x01' / '\x00\x02'/ '\x00\x02' / '\x00\x03'/ '\x80\x03' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst6
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:02:01:02', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x02' / '\x00\x02' / '\x00\x02' / '\x00\x03' / '\x80\x04' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst7
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:02:02:01', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x01' / '\x00\x02' / '\x00\x02' / '\x00\x04' / '\x80\x03' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst2
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:01:01:02', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x02' / '\x00\x01' / '\x00\x01' / '\x00\x03' / '\x80\x04' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst3
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:01:02:01', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x01' / '\x00\x01' / '\x00\x01' / '\x00\x04' / '\x80\x03' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst6
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:02:02:01', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x02' / '\x00\x04' / '\x80\x03' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())
	    ...
	    # dst7
            # r4.incr('send_data')
            pkt =  Ether(src=source, dst='00:00:00:02:02:02', type=1792)
            pkt = pkt / '\x00\x00' / '\x00\x01' / '\x00\x04' / '\x80\x04' 
            pkt = pkt / '\x08\x00'
            pkt = pkt / data
	    sendp(pkt, iface=iface, verbose=False)
            # time.sleep(sleep_time*random.random())

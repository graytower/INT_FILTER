        ...
        for i in xrange(set_num):
            for j in xrange(spine_num):
                for k in xrange(pod_num):
                    self.addLink(
                        self.spine_sw_list[i][j], self.leaf_sw_list[k][i],bw=BW/10)

        for i in xrange(pod_num):
            for j in xrange(leaf_num):
                for k in xrange(tor_num):
                    self.addLink(
                        self.leaf_sw_list[i][j], self.tor_sw_list[i][k],bw=BW)

        for i in xrange(pod_num):
            for j in xrange(tor_num):
                for k in xrange(h_num):
                    self.addLink(self.h_list[i][j][k], self.tor_sw_list[i][j],bw=BW*10)


def database_init(r,r2,r3,r4,W,nodes_list=[2,2,2,2,2]):
    r2.flushall()
    ...
    for key in keys:
        ...
        r2.lpush(key,0)
        for i in range(W):
            r2.rpush(key,'%d:-1'%(W-i))



def main():
    with open(os.path.abspath(os.path.join(os.getcwd(), ".."))+'/args','r') as f:
        W=int(f.readline())
